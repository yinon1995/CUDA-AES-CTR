// Type definitions for various unsigned integer sizes
typedef unsigned char			u8;
typedef unsigned short			u16;
typedef unsigned int			u32;
typedef unsigned long long	    u64;

// Kernel execution configuration constants
#define BLOCKS				256      // Number of blocks in the kernel
#define THREADS				128      // Number of threads per block
#define TWO_POWER_RANGE		30       // The key range is 2^30 for AES counter mode

// Shared memory configuration constants
#define SHARED_MEM_BANK_SIZE			32   // Number of banks in shared memory for T0 table
#define S_BOX_BANK_SIZE					8    // Number of banks in shared memory for S-box
#define TABLE_SIZE						256  // AES tables size (number of entries in T0-T4)
#define RCON_SIZE						10   // Number of round constants (RCON)

// AES-128 key configuration
#define AES_128_KEY_LEN_INT		4    // Key length in 32-bit unsigned integers (128 bits)
#define AES_128_KEY_SIZE_INT	44   // Expanded key size for AES-128 (11 * 4 = 44 32-bit words)

// AES round configuration
#define ROUND_COUNT				10   // Number of rounds in AES-128
#define ROUND_COUNT_MIN_1		9    // One less than the total number of rounds (for loops)
#define U32_SIZE						4
#define MAX_U32							4294967295

// Constants for byte permutation using __byte_perm() function
// The selector values determine how bytes are permuted within a 32-bit word
#define SHIFT_1_RIGHT			17185  // Select bytes for a shift to the right by 8 bits
#define SHIFT_2_RIGHT			21554  // Select bytes for a shift to the right by 16 bits
#define SHIFT_3_RIGHT			25923  // Select bytes for a shift to the right by 24 bits

// Macro for checking GPU errors after CUDA API calls
#define gpuErrorCheck(ans) { gpuAssert((ans), __FILE__, __LINE__); }
inline void gpuAssert(cudaError_t code, const char* file, int line, bool abort = true) {
	// If an error occurs, print the error message and file/line information
	if (code != cudaSuccess) {
		fprintf(stderr, "GPUassert: %s %s %d\n", cudaGetErrorString(code), file, line);
		// Optionally exit the program (commented out here for debugging purposes)
		// if (abort) exit(code);
	}
}

// Function to print the last CUDA error if any occurred
void printLastCUDAError() {
	cudaError_t cudaError = cudaGetLastError();
	// If there is a CUDA error, print its error code and message
...
// Function to calculate the key range for each thread in AES counter mode
__host__ u32* calculateRange() {
	u32* range;
	// Allocate memory for the range variable on the GPU
	gpuErrorCheck(cudaMallocManaged(&range, 1 * sizeof(u32)));

	// Calculate the total number of threads and the key range for each thread
	int threadCount = BLOCKS * THREADS;
	double keyRange = pow(2, TWO_POWER_RANGE);  // Total key range (2^30 in this case)
	double threadRange = keyRange / threadCount; // Key range per thread
	*range = ceil(threadRange);  // Set the key range per thread (rounded up)

	// Print the calculated ranges and thread information
	printf("Blocks                        : %d\n", BLOCKS);
	printf("Threads                       : %d\n", THREADS);
...
// Device function to perform arithmetic right shift for a 32-bit unsigned integer
__device__ u32 arithmeticRightShift(u32 x, u32 n) {
	// Perform right shift by n and circularly shift the bits to the left
	return (x >> n) | (x << (-n & 31));  // Ensure the shift amount is within bounds (0-31)
}

// Device function to perform a right shift for 16-bit sections within a 32-bit word
__device__ u32 arithmetic16bitRightShift(u32 x, u32 n, u32 n2Power) {
	// Perform right shift by n and circularly shift bits within the 16-bit sections
	return (x >> n) | ((x & n2Power) << (-n & 15));  // Ensure the shift stays within 16 bits
}

// Device function for byte permutation-based right shifts (using __byte_perm)
__device__ u32 arithmeticRightShiftBytePerm(u32 x, u32 n) {
	// Use __byte_perm to rearrange bytes in x based on the selector n
	return __byte_perm(x, x, n);  // Permutes bytes within the 32-bit word
}

// Macros for rotating a 64-bit or 16-bit value to the left by n bits
#define ROTL64(x,n) (((x)<<(n))|((x)>>(64-(n))))  // Rotate a 64-bit value left by n bits
#define ROTL16(x,n) (((x)<<(n))|((x)>>(16-(n)))) & 0xffff  // Rotate a 16-bit value left by n bits, masking to 16 bits
